...
class TelegramBotService {
  ...
  // ========== NUEVO: NODO DE PREGUNTAR Y GUARDAR ==========
  ...
  async stop() {
    try {
      await this.bot.stop();
      console.log(`✅ Bot ${this.botId} detenido`);
      await supabase.from('bots').update({ status: 'inactive' }).eq('id', this.botId);
      return true;
    } catch (error) {
